...
impact: HIGH
impactDescription: Prevents migration failures and enables idempotent auth changes
tags: constraints, migrations, auth, alter-table
---
...
begin
  if not exists (
    ...
    alter table auth add constraint check_age_positive check (age > 0);
  end if;
end $$;
