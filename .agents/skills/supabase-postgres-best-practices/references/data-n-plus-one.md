...
```sql
-- First query: get all auth
select id from auth where active = true;  -- Returns 100 IDs

-- Then N queries, one per auth
...
-- for auth in auth: db.query("SELECT * FROM orders WHERE user_id = $1", auth.id)
