...
- **`guides/auth-only.md`** - Schema creation and modification (see: Table of Contents)
...
- "Create a auth table with server fields" → Loads auth guide → Generates auth
...
- `templates/auth-example.ts` - Multi-table auth with relations
- `templates/drizzle-config.ts` - Configuration examples
- `scripts/generate-auth.ts` - Automated migration generation
