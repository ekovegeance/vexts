...
**What this does:**
- Compares auth.ts with database
...
```sql
-- 0001_add_phone_number.sql
ALTER TABLE auth ADD COLUMN phone_number VARCHAR(20);
```
...
**Generated:**
```sql
ALTER TABLE auth DROP COLUMN phone_number;
```
...
ALTER TABLE auth ADD COLUMN full_name VARCHAR(255);
```

**Solution:** Manually edit migration:
```sql
-- Change to:
ALTER TABLE auth RENAME COLUMN name TO full_name;
```
...
**Generated:**
```sql
ALTER TABLE auth ADD CONSTRAINT users_email_unique UNIQUE (email);
```
...
-- Populate with data
UPDATE auth SET full_name = first_name || ' ' || last_name;

-- Make not null after population
ALTER TABLE auth ALTER COLUMN full_name SET NOT NULL;
```
...
-- Copy data
UPDATE auth SET full_name = name;
```
...
DROP TABLE posts;     -- Then (depends on auth)
DROP TABLE auth;     -- Finally
```
...
-- Then drop column
ALTER TABLE auth DROP COLUMN your_column;
```
...
ALTER TABLE auth ADD COLUMN phone_number VARCHAR(20);
UPDATE auth SET phone_number = '000-000-0000' WHERE phone_number IS NULL;
ALTER TABLE auth ALTER COLUMN phone_number SET NOT NULL;
...
- ❌ Bad: "Add phone number, refactor auth table, update indexes"

## Related Resources

- `guides/troubleshooting.md` - Migration error solutions
- `guides/auth-only.md` - Schema change patterns
