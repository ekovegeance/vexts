...
```typescript
await db.batch([
  db.insert(auth).values({ email: 'test@example.com' }),
...
await db.transaction(async (tx) => {
  ...
  await tx.insert(posts)
    .values({ userId: auth[0].id, title: 'First post' });
});
```
...
export async function createUser(email: string) {
  return db.insert(auth).values({ email }).returning();
}
```
...
app.get('/health', async (req, res) => {
  try {
    await db.select().from(auth).limit(1);
